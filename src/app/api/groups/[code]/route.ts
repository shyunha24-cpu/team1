...
type StoredMessage = { id: string; userId?: string; text: string; message?: string; name: string; createdAt: string };
async function readMessages(root: string): Promise<StoredMessage[]> {
  const legacy = await read<StoredMessage[]>(`${root}/messages.json`, []);
  const result = await list({ prefix: `${root}/messages/`, limit: 1000 });
  const individual = await Promise.all(result.blobs.map(blob => read<StoredMessage | null>(blob.pathname, null)));
  const merged = new Map<string, StoredMessage>();
  for (const message of [...legacy, ...individual.filter((item): item is StoredMessage => Boolean(item))]) merged.set(message.id, message);
...
export async function GET(_: Request, { params }: { params: Promise<{ code: string }> }) {
  ...
  const [messages, routine] = await Promise.all([readMessages(root), read(`${root}/routine.json`, {})]);
...
export async function POST(request: Request, { params }: { params: Promise<{ code: string }> }) {
  ...
  if (body.type === "join") {
    ...
    const text = String(body.text ?? "").trim(); const name = String(body.name ?? "익명").trim().slice(0, 20) || "익명"; const userId = String(body.userId ?? "").trim().toLowerCase();
    if (!text) return NextResponse.json({ error: "메시지를 입력해 주세요." }, { status: 400 });
    const message: StoredMessage = { id: crypto.randomUUID(), userId, text: text.slice(0, 500), message: text.slice(0, 500), name, createdAt: new Date().toISOString() };
    await put(`${root}/messages/${Date.now()}-${message.id}.json`, JSON.stringify(message), { access: "private", addRandomSuffix: false, contentType: "application/json", cacheControlMaxAge: 0 });
    return NextResponse.json({ group, message });
    ...
    return NextResponse.json({ group, routine });
    ...
    const messageFiles = await list({ prefix: `${root}/messages/`, limit: 1000 });
    await del([`${root}/group.json`, `${root}/messages.json`, `${root}/routine.json`, ...messageFiles.blobs.map(blob => blob.url)]);
